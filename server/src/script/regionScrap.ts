...
const lists = [
  {
    do: "서울특별시",
    gu: "서대문구",
    dongList: ["충현동", "천연동", "북아현동", "신촌동", "연희동", "홍제2동", "홍은1동", "홍은2동"]
  },
  {
    do: "서울특별시",
    gu: "마포구",
    dongList: ["아현동", "공덕동", "도화동", "용강동", "대흥동", "염리동", "신수동", "서강동", "서교동", "합정동", "연남동", "성산1동", "성산2동", "상암동"]
  },
  {
    do: "서울특별시",
    gu: "은평구",
    dongList: ["녹번동", "구산동", "대조동", "역촌동", "신사1동", "신사2동", "증산동", "수색동", "진관동"]
  }
];
...
(async () => {
  ...
  for (const region of lists) {
    for (const dong of region.dongList) {
      ...
      if (coord) results.push({ do:region.do, gu: region.gu, dong, dongCoord:{...coord} });
    }
  }
  ...
  const guCoords = Object.entries(guGrouped).reduce((acc, [gu, list]) => {
    const avgLat = (list as Record<string, any>[]).reduce((sum, i) => sum + i.dongCoord?.lat , 0) / (list as Record<string, number>[]).length;
    const avgLon = (list as Record<string, any>[]).reduce((sum, i) => sum + i.dongCoord?.lon , 0) / (list as Record<string, number>[]).length;
    acc[gu] = { lat: avgLat, lon: avgLon };
    return acc;
  }, {} as Record<string, { lat: number; lon: number }>);

  console.log(guCoords)
